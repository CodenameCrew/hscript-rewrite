...
void TreeNode_obj::__construct( ::haxe::ds::TreeNode l, ::Dynamic k, ::Dynamic v, ::haxe::ds::TreeNode r,::hx::Null< int >  __o_h){
	...
	if ((h == -1)) {
		int _hx_tmp;
		 ::haxe::ds::TreeNode _this = this->left;
		int _hx_tmp1;
		if (::hx::IsNull( _this )) {
			_hx_tmp1 = 0;
		}
		else {
			_hx_tmp1 = _this->_height;
		}
		 ::haxe::ds::TreeNode _this1 = this->right;
		int _hx_tmp2;
		if (::hx::IsNull( _this1 )) {
			_hx_tmp2 = 0;
		}
		else {
			_hx_tmp2 = _this1->_height;
		}
		if ((_hx_tmp1 > _hx_tmp2)) {
			 ::haxe::ds::TreeNode _this2 = this->left;
			if (::hx::IsNull( _this2 )) {
				_hx_tmp = 0;
			}
			else {
				_hx_tmp = _this2->_height;
			}
		}
		else {
			 ::haxe::ds::TreeNode _this3 = this->right;
			if (::hx::IsNull( _this3 )) {
				_hx_tmp = 0;
			}
			else {
				_hx_tmp = _this3->_height;
			}
		}
		this->_height = (_hx_tmp + 1);
	}
	else {
		this->_height = h;
	}
...
::String TreeNode_obj::toString(){
	::String _hx_tmp;
	if (::hx::IsNull( this->left )) {
		_hx_tmp = HX_("",00,00,00,00);
	}
	else {
		_hx_tmp = (this->left->toString() + HX_(", ",74,26,00,00));
	}
	::String _hx_tmp1 = ((HX_("",00,00,00,00) + ::Std_obj::string(this->key)) + HX_(" => ",7f,53,55,15));
	::String _hx_tmp2 = (_hx_tmp + (_hx_tmp1 + ::Std_obj::string(this->value)));
	::String _hx_tmp3;
	if (::hx::IsNull( this->right )) {
		_hx_tmp3 = HX_("",00,00,00,00);
	}
	else {
		_hx_tmp3 = (HX_(", ",74,26,00,00) + this->right->toString());
	}
	return (_hx_tmp2 + _hx_tmp3);
}
