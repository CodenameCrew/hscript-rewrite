...
void PosException_obj::__construct(::String message, ::haxe::Exception previous, ::Dynamic pos){
	{
		super::__construct(message,previous,null());
		if (::hx::IsNull( pos )) {
			this->posInfos = ::hx::SourceInfo(HX_("(unknown)",87,5d,fc,5f),0,HX_("(unknown)",87,5d,fc,5f),HX_("(unknown)",87,5d,fc,5f));
		}
		else {
			this->posInfos = pos;
		}
