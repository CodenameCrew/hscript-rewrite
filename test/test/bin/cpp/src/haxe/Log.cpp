...
::String Log_obj::formatOutput( ::Dynamic v, ::Dynamic infos){
	::String str = ::Std_obj::string(v);
	if (::hx::IsNull( infos )) {
		return str;
	}
	::String pstr = ( (::String)(((infos->__Field(HX_("fileName",e7,5a,43,62),::hx::paccDynamic) + HX_(":",3a,00,00,00)) + infos->__Field(HX_("lineNumber",dd,81,22,76),::hx::paccDynamic))) );
	bool _hx_tmp;
	if (::hx::IsNotNull( infos )) {
		_hx_tmp = ::hx::IsNotNull( infos->__Field(HX_("customParams",d7,51,18,ed),::hx::paccDynamic) );
	}
	...
	if (_hx_tmp) {
		...
		while((_g < _g1->get_length())){
			 ::Dynamic v1 = _g1->__get(_g);
			_g = (_g + 1);
			str = (str + (HX_(", ",74,26,00,00) + ::Std_obj::string(v1)));
		}
	}
	return ((pstr + HX_(": ",a6,32,00,00)) + str);
}
