...
void FileOutput_obj::writeByte(int c){
	try {
		...
		_hx_std_file_write_char(this->_hx___f,c);
	} catch( ::Dynamic _hx_e) {
		if (_hx_e.IsClass<  ::Dynamic >() ){
			...
			HX_STACK_DO_THROW(::haxe::io::Error_obj::Custom(_g));
		}
		else {
...
int FileOutput_obj::writeBytes( ::haxe::io::Bytes s,int p,int l){
	try {
		HX_STACK_CATCHABLE( ::Dynamic, 0);
		return _hx_std_file_write(this->_hx___f,s->b,p,l);
	} catch( ::Dynamic _hx_e) {
		if (_hx_e.IsClass<  ::Dynamic >() ){
			HX_STACK_BEGIN_CATCH
			 ::Dynamic _g = _hx_e;
...
void FileOutput_obj::flush(){
	_hx_std_file_flush(this->_hx___f);
}
